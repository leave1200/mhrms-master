...
class Attendance extends CI_Controller {
    ...
    public function Add_Attendance() {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $status = $this->input->post('status');  // Add this line to get the status input
                        ...
                        if ($holiday) {
                            ...
                            if (!empty($duplicate)) {
                                ...
                                $data = array(
                                    ...
                                    'working_hour' => $work,
                                    'place' => $place,
                                    'status' => $status // Use the status provided
                                );
                                $this->attendance_model->Add_AttendanceData($data);
                                echo "Successfully Added.";
                                
                                // Deduct leave balance if status is LWP or CL
                                if ($status == 'LWP' || $status == 'CL') {
                                    $this->deductLeaveDays($em_id, $status);
                                }
                            }
                        }
                    }
    ...
    private function deductLeaveDays($em_id, $leave_type) {
        // Fetch the current leave balance for the employee
        $leave_balance = $this->leave_model->getLeaveBalance($em_id, $leave_type);

        if ($leave_balance > 0) {
            // Deduct one day from the leave balance
            $new_balance = $leave_balance - 1;

            // Update the leave balance in the database
            $this->leave_model->updateLeaveBalance($em_id, $leave_type, $new_balance);

            echo "Leave balance updated.";
        } else {
    ...
    private function isHoliday($date) {
        $holiday = $this->leave_model->get_holiday_between_dates($date);
