   ...
   /* public function Salary_List(){
        ...
        if($this->session->userdata('user_login_access') != False) { 
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data2 = array(
                    ...
                    'bima' => $bima,
                    'tax' => $tax, // Assign calculated tax amount here
                    'others' => $others
                );
    ...
    public function Invoice(){
       ...
       $month_holiday_count = $this->payroll_model->getNumberOfHolidays($month, $year);
        ...
        $daily_rate = $employee_salary / $total_work_days;
        $weekly_rate = $employee_salary / ($month / 7);

        $work_hour_diff = abs($total_work_hours) - abs($employee_actually_worked[0]->Hours);
        


        $work_hour_diff = abs(intval($total_work_hours)) - abs(intval($employee_actually_worked[0]->Hours));
