...
class Leave extends CI_Controller
{
    ...
    public function getLeaveBalance($employee_id, $leave_type) {
        $this->db->select('leave_balance');
        $this->db->from('leave_balance');
        $this->db->where('employee_id', $employee_id);
        $this->db->where('leave_type', $leave_type);
        $query = $this->db->get();
        return $query->row()->leave_balance;
    }

    // Method to update the leave balance for an employee
    public function updateLeaveBalance($employee_id, $leave_type, $new_balance) {
        $this->db->where('employee_id', $employee_id);
        $this->db->where('leave_type', $leave_type);
        $this->db->update('leave_balance', array('leave_balance' => $new_balance));
    }

    // Method to check if a given date is a holiday
    public function get_holiday_between_dates($date) {
        $this->db->select('*');
